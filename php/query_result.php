	...
	foreach ($usable_table -> category as $size){
		...
		switch ($usable_table -> category[$i]){
			...
			case 'All':
				...
				$query_line .= " (repos.repos_name LIKE ".$like." OR umlfiles.umlfiles_name LIKE ".$like." OR repos.founder LIKE ".$like.")";
				
				//build the query to display the number files per projet
				if(strpos($query_line,'COUNT(umlfiles.id) AS count_filenumber') == false  && $display == "'project'"){
					...
					$query_line =substr_replace($query_line, "",strpos($query_line,", COUNT(umlfiles.id) AS count_filenumber "),-1);
				}
				if($i == sizeof($usable_table -> category)-1 && $display == "'project'"){
					$query_line .= " GROUP BY repos.id";
				}
							
				//build the query to display the number of xmi
			...
			case 'Author name':
				...
				//build the query to display the number of xmi files
				if(strpos($query_line,"has_xmi") == false){
			...
			case 'File name':
					...
					//if this is chosen, the results are diagram, so no count of files per project
			...
			case 'Project name':
				...
				if(strpos($query_line,"has_xmi") == false){
					...
					$query_line = substr_replace($query_line, "  INNER JOIN (SELECT repos.id AS test_repos_id, COUNT(umlfiles.has_xmi) AS count_xmi FROM umlfiles INNER JOIN repos ON repos.id = umlfiles.repo_id WHERE repos_name LIKE ".$like." GROUP BY repos.id) AS test ON test_repos_id = umlfiles.repo_id ",strpos($query_line,'WHERE')-1,0);
				
				}else if(isset($usable_table -> andor[$i-1]) && $usable_table -> andor[$i-1] == "AND" && strpos($query_line,"has_xmi") != false){
					
					$query_line = substr_replace($query_line, "AND repos_name LIKE ".$like." ",strpos($query_line,"GROUP BY repos.id"),0);
				
				}else if(isset($usable_table -> andor[$i-1]) && $usable_table -> andor[$i-1] == "OR" && strpos($query_line,"has_xmi") != false){
					$query_line = substr_replace($query_line, "OR repos_name LIKE ".$like." ",strpos($query_line,"GROUP BY repos.id"),0);
		...
		// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		// Add the 'and' or 'or' when it exists 
		// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		if(isset($usable_table -> andor[$i]) && $usable_table -> andor[$i] == "AND"){
			$query_line .= " AND ";
		}else if(isset($usable_table -> andor[$i]) && $usable_table -> andor[$i] == "OR"){
			$query_line .= " OR ";
		}
